...
export const anonymizeResponses = async (survey) => {
    ...
    const anonResponsesFile = path.join(
        'data',
        'normalized_responses_anon',
        `${surveyName}_normalized_responses_anon.ndjson`
    )
    ...
    await writeIterableToFile(
        ndJsonFromAsyncIterator(
            anonymize(responseCount, ndJsonToObject(chunksToLine(responsesReader)))
        ),
        anonResponsesFile
    )
