...
export const cleanupMongoResponses = async (filter = {}) => {
    ...
    const collection = await getCollection(client, 'normalized_responses')
...
export const saveResponsesToMongo = async (survey) => {
    ...
    console.log(`[${surveyName}] saving ${responseCount} responses to mongo from '${responsesFile}'`)

    const client = getClient()
    const collection = await getCollection(client, 'normalized_responses_3')
