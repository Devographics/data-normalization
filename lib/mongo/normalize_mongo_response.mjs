...
export const normalizeMongoResponse = (survey, response) => {
  ...
  const keysToNormalize = [
    ...
    },
    {
      ...
      keys: ['resources.blogs_news_magazines.others', 'resources.podcasts.others', 'resources.sites_courses.others'],
    },
  ];
  ...
  keysToNormalize.forEach(section => {
    ...
    keys.forEach(key => {
      ...
      if (value) {
        ...
        _.set(normalized, `${key}_normalized`, normalizedValues.map(v => v[0]));
        _.set(normalized, `${key}_patterns`, normalizedValues.map(v => v[1].toString()));
      }
    });
  });
