...
const TYPEFORM_FIELD_TYPE_OPINION_SCALE = 'opinion_scale'
const TYPEFORM_FIELD_TYPE_LONG_TEXT = 'long_text'
...
export const extractTypeformConfig = async (survey) => {
    ...
    globalOpinionsField.properties.fields.forEach((field) => {
        ...
        if (field.type === TYPEFORM_FIELD_TYPE_OPINION_SCALE) {
            config.fields[field.id] = {
                ...config.fields[field.id],
                type: fieldTypes.FIELD_TYPE_GLOBAL_OPINION,
                subject,
            }
        } else if (field.type === TYPEFORM_FIELD_TYPE_LONG_TEXT) {
            config.fields[field.id] = {
                ...config.fields[field.id],
                type: fieldTypes.FIELD_TYPE_OTHER_OPINION,
                subject,
            }
        } else {
            throw new Error(`unknow opinion field`, field)
        }
    })
    ...
    Object.keys(config.sections).forEach((sectionId) => {
        ...
        sectionField.properties.fields.forEach((field) => {
            ...
            if (sectionId === sections.ENVIRONMENTS) {
                if (field.type === TYPEFORM_FIELD_TYPE_MULTI_CHOICE) {
                    const environmentId = normalizeEnvironmentId(field.title)
                    config.fields[field.id] = {
                        ...config.fields[field.id],
                        type: field.properties.allow_other_choice
                            ? fieldTypes.FIELD_TYPE_ENVIRONMENT_CHOICES
                            : fieldTypes.FIELD_TYPE_ENVIRONMENT_RATING,
                        environment: environmentId,
                    }
                    // handle options converted to rating
                    if (field.properties.choices.length === 4) {
                        config.fields[field.id].rating_mapping = field.properties.choices.map(
                            (choice, rating) => {
                                return {
                                    label: choice.label,
                                    rating,
                    ...
                    sectionEnvironments.push(environmentId)
                    return
                }

                console.warn(`unknown environment field:`, field)
                return
            }
        ...
        if (sectionEnvironments.length > 0) {
            sectionConfig.environments = sectionEnvironments
        }
    })
    ...
    const proficiencyFields = aboutField.properties.fields.filter((field) =>
        field.title.endsWith('Proficiency')
    )
    proficiencyFields.forEach((proficiencyField) => {
        const proficiencyId = normalizeProficiencyId(proficiencyField.title)
        config.fields[proficiencyField.id] = {
            ...config.fields[proficiencyField.id],
            type: fieldTypes.FIELD_TYPE_PROFICIENCY,
            proficiency: proficiencyId,
            proficiency_mapping: proficiencyField.properties.choices.map((choice, rating) => {
                return {
                    label: choice.label,
                    rating,
                }
    ...
    // compute job title
    const jobTitleField = aboutField.properties.fields.find((field) => field.title === 'Job Title')
    if (jobTitleField !== undefined) {
        config.user_info.push(fieldTypes.FIELD_TYPE_JOB_TITLE)
        config.fields[jobTitleField.id] = {
            ...config.fields[jobTitleField.id],
            type: fieldTypes.FIELD_TYPE_JOB_TITLE,
        }
    }
