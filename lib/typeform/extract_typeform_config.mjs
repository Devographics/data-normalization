...
import { SECTION_TYPE_USED_FEATURES_BY_CATEGORY } from '../constants.mjs'
import { globalOpinionsSubjectNormalizers, normalizeTool, normalizeFeatureId } from '../normalize.mjs'
...
export const extractTypeformConfig = async (survey) => {
    ...
    Object.keys(config.sections).forEach(sectionId => {
        ...
        const sectionFeaturesUsedByCategory = []
        ...
        sectionField.properties.fields.forEach(field => {
            ...
            if (sectionConfig.type === SECTION_TYPE_USED_FEATURES_BY_CATEGORY) {
                if (field.type === TYPEFORM_FIELD_TYPE_MULTI_CHOICE) {
                    const featureId = normalizeFeatureId(field.title)
                    config.fields[field.id] = {
                        ...config.fields[field.id],
                        type: fieldTypes.FIELD_TYPE_FEATURES_USED_BY_CATEGORY,
                        feature: featureId,
                        section: sectionId,
                    }
                    sectionFeaturesUsedByCategory.push(featureId)
                    config.features_used_by_category.push(featureId)
                    return
                }
            }
        ...
        sectionConfig.features_used_by_category = sectionFeaturesUsedByCategory
    })
