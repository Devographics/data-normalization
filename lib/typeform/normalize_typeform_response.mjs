...
export const normalizeTypeformResponse = async (survey, { answers, ...response }) => {
    const normalized = {
        ...
        user_info: {
            browser_type:
                response.metadata.browser === 'undefined' ? undefined : response.metadata.browser,
            user_agent:
                response.metadata.user_agent === 'undefined'
                    ? undefined
                    : response.metadata.user_agent,
            platform:
                response.metadata.platform === 'undefined' ? undefined : response.metadata.platform,
        },
        tools: {},
        tools_others: {},
        features: {},
    ...
    if (response.hidden !== undefined) {
        ...
        if (response.hidden.source && response.hidden.source !== 'undefined') {
            hiddenSource = response.hidden.source
        }
    }

    const referrer = response.metadata.referer
    if (referrer) {
        normalized.user_info.referrer = response.metadata.referer
    }
    ...
    answers.forEach((answer) => {
        ...
        switch (fieldConfig.type) {
            ...
            case fieldTypes.FIELD_TYPE_SOURCE:
                ...
                // try to use hidden source field first
                if (hiddenSource) {
                    const normalizedHiddenSource = normalizeSource(hiddenSource)
                    if (isValidNormalization(normalizedHiddenSource, sourceNormalizationRules)) {
                        rawSource = hiddenSource
                        normalizedSource = normalizedHiddenSource
                    }
                }

                // then try to use the freeform
                if (!normalizedSource) {
                    const sourceText = cleanupValue(answer.text.trim().toLowerCase())
                    if (sourceText) {
                        const normalizedSourceText = normalizeSource(sourceText)
                        if (isValidNormalization(normalizedSourceText, sourceNormalizationRules)) {
                            rawSource = sourceText
                            normalizedSource = normalizedSourceText
                        }
                    }
                }

                // finally, try guessing it from the referrer
                if (!normalizedSource && referrer) {
                    // referrer is typeform, we extract the referrer contained
                    // in it, available as a query param.
                    const referrerQueryParams = querystring.parse(referrer)
                    if (referrerQueryParams && referrerQueryParams.referrer) {
                        const cleanedUpReferrer = cleanupValue(referrerQueryParams.referrer)
                        if (cleanedUpReferrer) {
                            const normalizedReferrerSource = normalizeSource(cleanedUpReferrer)
                            if (
                                isValidNormalization(
                                ...
                                rawSource = cleanedUpReferrer
                                normalizedSource = normalizedReferrerSource
                            }
                        }
                    }
                }
